...
export default class App extends React.Component {
  ...
  render() {
    if(!this.state.selected){
      return(
        <div>
          ...
          <form>
            ...
            <div className="container">
              <span>
                <label className="checkbox">
                  <input type="radio" name="is_multi" checked={this.state.is_multi} onChange={this.toggleCheckbox} />
                  <span className="checkbox-label">Multi player</span>
                </label>
                &nbsp;
                <label className="checkbox">
      						<input type="radio" name="is_single" checked={this.state.is_single} onChange={this.toggleCheckbox} />
              ...
              <label className="checkbox">
                <input type="checkbox" className="checkbox-control" name="is_hard" checked={this.state.is_hard} onChange={this.toggleCheckbox} />
            ...
            <div className="scontainer"></div>
              <button className="registerbtn" onClick={this.handle}>Done</button>
        ...
        if(this.state.is_single){
            ...
            return(
              <div>{this.state.score_a>=this.state.score_b?<div>{
                  this.state.score_a==this.state.score_b?"Draw!!!":"Player A Wins!!!"
                }</div>
              :"Player B Wins!!!"}
      ...
      return(
      <div>
        <button className="registerbtn" onClick={this.reset_game}>Reset Game</button>
          <button className="registerbtn" onClick={this.handleClick }>
            Back to Menu
          </button>

        <span><strong>Current Score </strong></span>&nbsp; (
        <span><strong>Player1:</strong> {this.state.score_a}&nbsp;</span>
        {this.state.is_multi?<span><strong>Player2:</strong> {this.state.score_b}</span>:""})

        <div><strong>Turn</strong> <span>{this.state.turn}</span></div>

      {this.state.is_multi?
          <div><strong>Current Player is</strong> {this.state.turn%2==0?"A":"B"}</div>:""
        }

        <div><Timer /></div>
        <div><strong>Best Score: </strong> {this.state.score_best}</div>
        <hr/>
        {
         this.state.cards.map((card, i) => {
            return (
              (card.revealed ?
                (<div className="card_up" key={i} onClick={(e)=>{
                    if (!card.revealed) {
                      this.update(i);
                    }
                  }}>
              ...
              </div>): (<div className="card_down" key={i} onClick={(e)=>{
                    if (!card.revealed) {
                      this.update(i);
                    }
